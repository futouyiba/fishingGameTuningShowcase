...
- Strong Bake Manifest blockers (SB-01…SB-08): a carrier that materialized semantic consequences fails closed without a manifest (`BAKE_MANIFEST_MISSING`); duplicate settlement by semantic identity is revision-blind (`DUPLICATE_SEMANTIC_CONSEQUENCE`); same raw fact under distinct cause roles stays legal; invalidation is replacement, never stacking; required lineage fields (`BAKE_LINEAGE_INCOMPLETE`) and unknown validity (`BAKE_VALIDITY_UNKNOWN`) fail closed before the numeric kernel; stage summaries are cross-checked against manifest coverage (`BAKE_STAGE_SUMMARY_MISMATCH`); attaching the manifest leaves the pre-existing partial/full bake numbers unchanged.
